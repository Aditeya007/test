...
exports.createUser = async (req, res) => {
  const { name, email, username, password, botCount, websites } = req.body;
  ...
  try {
    ...
    const count = parseInt(botCount, 10) || 1;

    // Validate botCount range
    if (count < 1 || count > 10) {
      return res.status(400).json({ error: 'botCount must be between 1 and 10', field: 'botCount' });
    }

    // Validate websites array if provided
    if (websites !== undefined) {
      if (!Array.isArray(websites)) {
        return res.status(400).json({ error: 'websites must be an array', field: 'websites' });
      }
      if (websites.length < count) {
        return res.status(400).json({ 
          error: `websites array must contain at least ${count} URLs (one per bot)`, 
          field: 'websites' 
        });
      }
    }
    ...
    for (let i = 1; i <= count; i++) {
      const botName = count > 1 ? `Bot ${i}` : sanitizedName;
      const websiteUrl = websites?.[i - 1] || null;
      
      const bot = new Bot({
        userId: user._id,
        name: botName,
        vectorStorePath: '', // Will be set by provisioning
        apiToken: '', // Will be auto-generated by pre-save hook
        scrapedWebsites: websiteUrl ? [websiteUrl] : [],
        isActive: true
      });
      ...
      try {
        ...
        console.error(`❌ Bot ${i} provisioning failed:`, botProvisionErr);
        return res.status(500).json({ error: `Failed to provision resources for Bot ${i}` });
      }

      await bot.save();
      console.log(`✅ Bot created: ${botName} (${i}/${count})${websiteUrl ? ` - Website: ${websiteUrl}` : ''}`);
